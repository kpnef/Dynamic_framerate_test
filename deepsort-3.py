print("=====  采用 动态 跳帧模型，如果上一次检测的的框，和本次检测的框，存在有框的交并比低于某个阈值，跳帧级别下降，反之，则上升。加入前缓存队列，包含补充回溯检测  =====")
...
for i in range(1, 15):   # range(1, 15) 产生 1~14
    ...
    ##############  第一次 DEEPSORT 计算, 计算每帧所需要的 LEVEL 级别 #####################
    deepsort = DeepSort(
    ...
    for frame_id in sorted(frame_dict):
        ...
        if frame_id%v == 0 and len(dets) != 0:
            # 观察当前帧检测结果以及30MS 之前的预测数据
            det1   = to_xyxy(dets[:, :4])
            #print(det1)
            #print("det"+str(len(det1)))
            #print(lastOut)
            #print("out"+str(len(lastOut)))
        ...
        #print(len(lastOut))
        #print(lastOut)
        #print("lastOutLen="+str(len(lastOut)))
        #print("output")
        #print(outputs)
        #print(str(len(dets))+"  "+str(len(outputs))+"  "+str(len(bbox_xywh)))
        # DeepSort 返回: [x1,y1,x2,y2,track_id]
        
        levelA.append([frame_id, level])

    ############ 处理 LEVEL 结果
    level_dict = {f: lvl for f, lvl in levelA}          # {frame_id: level}
    last_frame = max(level_dict)                        # 序列最后一帧编号
    #print(level_dict)


    ##############  第二次 DEEPSORT 计算，根据上一次的 LEVEL，计算当前的 LEVEL 级别  #####################
    deepsort = DeepSort(
        model_path="deep_sort_pytorch-master/deep_sort/deep/checkpoint/ckpt.t7",max_age=70,min_confidence=0,n_init=1, use_cuda=False
    )
    ...
    for frame_id in sorted(frame_dict):
        ...
        # ===== ① 取当前帧起向后(未来) 15 帧内的最大 LEVEL =========
        end_id   = min(last_frame, frame_id + 15)       # 最多到未来 14 帧，加本帧共 15
        future_lvls = [
            level_dict[f] for f in range(frame_id, end_id + 1)
            if f in level_dict
        ]
        max_lvl = max(future_lvls) if future_lvls else 4   # 没记录就默认 4
        #print(max_lvl)
        v = get_v(max_lvl)
        if frame_id%v == 0 and len(dets) != 0:
            #  bbox_xywh 需为 (cx,cy,w,h)，开始预测本帧
